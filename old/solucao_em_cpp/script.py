...
def pontos (nome_problema):
	print ''
	if not os.path.isfile(nome_problema+".cpp"): 
		print nome_problema + ': 0'
		return 0
	print nome_problema
	os.system("g++ " + nome_problema + ".cpp")
	total = 0		
	max_point = 0	
	for i in range (1, 11):
		pontuacao_teste = 0
		...
		for j in range (1, 11):
			...
			point = True	
			command = Command("./a.out < " + dir + "in" + str(j) + " > out")
...
pontuacao = 0
pontuacao += pontos ("cor")
pontuacao += pontos ("coral")
pontuacao += pontos ("divisores")
pontuacao += pontos ("metro")
pontuacao += pontos ("quebra")
